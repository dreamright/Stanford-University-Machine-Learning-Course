...
%
%% Forward prop
%transform y into matrix [m x s2] ([1 0 0 0 0...; 0 0 1...;...]
...
%calculate J unregularized:
J = 1/m * sum(sum(-y1.*log(a3)-(1-y1).*log(1-a3)));

%regularization w/out Theta_0:
J = J + lambda/(2*m) *(sum(sum(Theta1(:,2:end).^2))+...
    sum(sum(Theta2(:,2:end).^2)));

%% Backprop
%calcutale delta by def (skipping bias thetas):
delta3 = a3 - y1;
delta2 = delta3 * Theta2(:,2:end) .* sigmoidGradient(X*Theta1');
%calculate theta gradients (T to equalize sizes w/ Theta):
